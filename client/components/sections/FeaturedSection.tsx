  ...
  return (
    <section className="relative py-12 md:py-24 overflow-hidden bg-black">
      <div className="absolute inset-0">
        <img src="/Arabic Cook/8.png" alt="" className="absolute inset-0 w-full h-full object-cover opacity-25" />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-16 items-center">
          <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="space-y-4 md:space-y-6" dir="rtl">
            <span className="inline-block px-3 py-1 bg-red-600/20 border border-red-500 text-red-400 text-xs font-bold rounded-full tracking-widest uppercase">وصفة مميزة</span>
            <h2 className="text-4xl md:text-6xl font-cinematic text-white leading-tight">مجموعة الوصفات العربية</h2>
            <p className="text-gray-400 text-base md:text-lg leading-relaxed">اكتشف أشهى الوصفات العربية الأصيلة من كباب وكفتة ودجاج وأكثر. استمتع بتجربة طهي لا مثيل لها مع أفضل الشيفات.</p>
            <div className="flex flex-wrap items-center gap-3 md:gap-6 text-gray-400 text-sm">
              <span className="flex items-center gap-2"><Clock className="w-4 h-4 text-red-500" /> 30 دقيقة</span>
            ...
            <div className="flex gap-3 pt-1">
              <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="flex items-center gap-2 px-5 py-3 md:px-8 md:py-4 bg-red-600 text-white font-bold rounded-lg hover:bg-red-700 transition-all shadow-lg shadow-red-600/30 text-sm md:text-base">
                <Play className="w-4 h-4 md:w-5 md:h-5 fill-white" /> مشاهدة
              </motion.button>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.92 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="rounded-2xl overflow-hidden shadow-2xl">
            <VideoCard image="/Arabic Cook/8.png" video={VIDEOS[8]} title="هالصينية بتختصر كل شي طيب" className="relative aspect-video rounded-2xl">
